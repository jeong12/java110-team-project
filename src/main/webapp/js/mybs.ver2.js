	    ...
	    oncomplete: function(data) {
	        ...
	        geocoder.addressSearch(data.address, function(results, status) {
	            ...
	            if (status === daum.maps.services.Status.OK) {
	                ...
	                // 해당 주소에 대한 좌표를 받아서
	                var editcoords = new daum.maps.LatLng(editresult.y, editresult.x);
	                // 지도를 보여준다.
            ...
            if(data ==false){
                ...
                if(data.supporter==null){// 개인스케줄의 경우 supporter 객체를 받지 않음, 고로 개인스케줄 모달에 데이터 처리
                    ...
                    $('#detailperModal').on('shown.bs.modal', function (e) {
                    	...
                    	var moveLatLon = new daum.maps.LatLng(data.x, data.y);
                    	permap.setCenter(moveLatLon);
                        permarker.setPosition(moveLatLon);
                    });
                    ...
                    $('#detailreqModal').on('shown.bs.modal', function (e) {
                    	...
                    	var moveLatLon = new daum.maps.LatLng(data.supporter.x, data.supporter.y);
                    	reqmap.setCenter(moveLatLon);
